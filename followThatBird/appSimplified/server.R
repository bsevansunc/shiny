...
# biomesR <- raster('biomesR')

# Get agriculture layer:

# agriculture <- raster('agIntensity33')
...
basemap <- leaflet(width = "50%", height="100%") %>%
  ...
  # addRasterImage(
  #   biomesR, colors = factPal, opacity = .7, project = FALSE,
  #   group = 'Biomes') %>%
  # addRasterImage(
  #   agriculture, colors = pal, project = FALSE,
...
server <- function(input, output, session) {
  ...
  # Tracking table output:
  
  output$tableQuery <- DT::renderDataTable(
    rownames = FALSE,
      isolate({
      trackingTableR()
      })
  ...
  tableProxy <- dataTableProxy('tableQuery')
  
  # Update tracking table:

  observe({
    replaceData(tableProxy, trackingTableR(), rownames = FALSE)
  })
  ...
  # Reactive values for tracking map:
  
  trackingMapR <- reactive({
    tracksOut <- tracks
  ...
  lineSubs <- reactive({
    ...
    if(!is.null(input$sppMap)){
      if(input$sppMap != 'Show all'){
        linesOut <- subset(linesFull, birdID %in% trackingMapR()$birdID)
      } 
      input$birdIDMap
      if(!is.null(input$birdIDMap)){
        if(input$birdIDMap != 'Show all'){
          linesOut <- subset(linesOut, birdID %in% input$birdIDMap)
        }
      }
    }
    linesOut
  ...
  observeEvent(input$mapIt, {
    ...
    # Update map with points and lines:
  ...
  #   points <- trackingMapR()
  #   mapOut <- leafletProxy('mapOut', data = points) %>% 
  #     clearControls()
